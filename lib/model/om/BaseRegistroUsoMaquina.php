...
abstract class BaseRegistroUsoMaquina extends BaseObject  implements Persistent {
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->rum_platos_teoricos = ($row[$startcol + 78] !== null) ? (string) $row[$startcol + 78] : null;
			$this->rum_tiempo_retencion = ($row[$startcol + 79] !== null) ? (string) $row[$startcol + 79] : null;
			$this->rum_resolucion = ($row[$startcol + 80] !== null) ? (string) $row[$startcol + 80] : null;
			$this->rum_tailing = ($row[$startcol + 81] !== null) ? (string) $row[$startcol + 81] : null;
			$this->resetModified();
			...
			return $startcol + 82; // 82 = RegistroUsoMaquinaPeer::NUM_COLUMNS - RegistroUsoMaquinaPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			...
			$keys[78] => $this->getRumPlatosTeoricos(),
			$keys[79] => $this->getRumTiempoRetencion(),
			$keys[80] => $this->getRumResolucion(),
			$keys[81] => $this->getRumTailing(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[78], $arr)) $this->setRumPlatosTeoricos($arr[$keys[78]]);
		if (array_key_exists($keys[79], $arr)) $this->setRumTiempoRetencion($arr[$keys[79]]);
		if (array_key_exists($keys[80], $arr)) $this->setRumResolucion($arr[$keys[80]]);
		if (array_key_exists($keys[81], $arr)) $this->setRumTailing($arr[$keys[81]]);
	}
