...
abstract class BaseRegistroUsoMaquina extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the rum_columnas field.
	 * @var        string
	 */
	protected $rum_columnas;
	...
	/**
	 * Get the [rum_columnas] column value.
	 * 
	 * @return     string
	 */
	public function getRumColumnas()
	{
		return $this->rum_columnas;
	}
	...
	/**
	 * Set the value of [rum_columnas] column.
	 * 
	 * @param      string $v new value
	 * @return     RegistroUsoMaquina The current object (for fluent API support)
	 */
	public function setRumColumnas($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}

		if ($this->rum_columnas !== $v) {
			$this->rum_columnas = $v;
			$this->modifiedColumns[] = RegistroUsoMaquinaPeer::RUM_COLUMNAS;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->rum_columnas = ($row[$startcol + 60] !== null) ? (string) $row[$startcol + 60] : null;
			...
			$this->rum_platos_teoricos = ($row[$startcol + 79] !== null) ? (string) $row[$startcol + 79] : null;
			$this->rum_tiempo_retencion = ($row[$startcol + 80] !== null) ? (string) $row[$startcol + 80] : null;
			$this->rum_resolucion = ($row[$startcol + 81] !== null) ? (string) $row[$startcol + 81] : null;
			$this->rum_tailing = ($row[$startcol + 82] !== null) ? (string) $row[$startcol + 82] : null;
			$this->resetModified();
			...
			return $startcol + 83; // 83 = RegistroUsoMaquinaPeer::NUM_COLUMNS - RegistroUsoMaquinaPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			...
			$keys[79] => $this->getRumPlatosTeoricos(),
			$keys[80] => $this->getRumTiempoRetencion(),
			$keys[81] => $this->getRumResolucion(),
			$keys[82] => $this->getRumTailing(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[60], $arr)) $this->setRumColumnas($arr[$keys[60]]);
		...
		if (array_key_exists($keys[79], $arr)) $this->setRumPlatosTeoricos($arr[$keys[79]]);
		if (array_key_exists($keys[80], $arr)) $this->setRumTiempoRetencion($arr[$keys[80]]);
		if (array_key_exists($keys[81], $arr)) $this->setRumResolucion($arr[$keys[81]]);
		if (array_key_exists($keys[82], $arr)) $this->setRumTailing($arr[$keys[82]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(RegistroUsoMaquinaPeer::RUM_COLUMNAS)) $criteria->add(RegistroUsoMaquinaPeer::RUM_COLUMNAS, $this->rum_columnas);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setRumColumnas($this->rum_columnas);
